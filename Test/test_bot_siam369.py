...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "SIAM369_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('SIAM369Bot')
    ...
    logger.info("=" * 60)
    logger.info("SIAM369 PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.siam369th.com/en-th"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.siam369th.com/en-th", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await page.screenshot(path="SIAM369_%s_%s_Payment_Page.png"%(deposit_method,deposit_channel),timeout=30000)
                break 
            except TimeoutError:
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """
            files = glob.glob("*SIAM369_%s_%s*.png"%(deposit_method,deposit_channel))
        ...
        fail_msg = (
                "⚠️ *SIAM369 RETRY 3 TIMES FAILED*\n"
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*SIAM369*.png")
    ...
    if excel_len != 0:
        ...
        try:
            if os.path.exists(file):
                ...
                if "S369T" in sheets:
                    df = pd.read_excel(file,sheet_name="S369T")
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='S369T', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets S369T not found in file :%s"%file)
