...
async def telegram_send_operation(telegram_message,failed_reason, program_complete):
    ...
    jr_bing_chat_id = os.getenv("JR_BING_CHAT_ID")
    bot = Bot(token=TOKEN)
...
├─ **TimeOccurred:** `{timestamp}` """
            ...
            if 10 <= current_hour < 22:
               jr_bing_tag = "[Peemai](tg://user?id=5690463304), [NUDEE](tg://user?id=5443501630), [Namwan](tg://user?id=7020014599), [Yati](tg://user?id=6474451548), [Jmoys0606](tg://user?id=6163105212) "
                ...
                jr_bing_tag = "[Firstsee786](tg://user?id=6612621888), [jelly_haha](tg://user?id=5590096249), [Bombam90](tg://user?id=6837937009), [aree899](tg://user?id=5542026734)" 
            ...
            if status != 'deposit success':
                ...
                for attempt in range(3):
                    try:
                        with open(file_path, 'rb') as f:
                              await bot.send_photo(
                                    chat_id=jr_bing_chat_id,
                                    photo=f,
                                    caption=jr_bing_caption,
                                    parse_mode='MarkdownV2',
                                    read_timeout=30,
                                    write_timeout=30,
                                    connect_timeout=30
...
{summary_body}"""
    ...
    for attempt in range(3):
        try:
            await bot.send_message(chat_id=jr_bing_chat_id, text=caption, parse_mode='MarkdownV2', disable_web_page_preview=True)
            log.info("SUMMARY SENT")
            break
        except TimedOut:
            log.warning(f"TELEGRAM TIMEOUT，RETRY {attempt + 1}/3...")
        ...
        try:
            if os.path.exists(file):
                ...
                if "S5T" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="S5T")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='S5T', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets S5T not found in file :%s"%file)
