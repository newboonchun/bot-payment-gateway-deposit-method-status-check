...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "22WIN_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('22WINBot')
    ...
    logger.info("=" * 60)
    logger.info("22WIN PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.22winth9.com/en-th"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.22winth9.com/en-th", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    # class DOM: <button data-v-02e4819d="" type="button" class="topbar_btn_1" aria-label="Login">Login</button>
    try:
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await page.screenshot(path="22WIN_%s_%s_Payment_Page.png"%(deposit_method,deposit_channel),timeout=30000)
                break 
            except TimeoutError:
    ...
    try:
        for _ in range(20):
            ...
            if await toast.count() > 0:
                toast_exist = True
                await page.screenshot(path="22WIN_%s_%s_Payment_Page.png"%(deposit_method_text,deposit_channel),timeout=30000)
...
async def perform_payment_gateway_test(page):
    ...
    try:
        ...
        for i in range(deposit_method_total_count):
            ...
            # class DOM for deposit channel text & deposit amount input range text
            #<div class="deposit-meth pb-4 md:pb-5">
            ...
            #                <div class="deposit_select_bank_list_right">
            #                     <div class="deposit_select_bank_list_amount">200 - 1,000,000 THB</div></div>

            btn = deposit_method_button.nth(i)
            deposit_method = await btn.locator('span.deposit_payment_method').inner_text()

            deposit_channel = await btn.locator('div.deposit_select_bank_list_name').inner_text()

            deposit_amount_input_range = await btn.locator('div.deposit_select_bank_list_amount').inner_text()
            ...
            try:
                ...
                # DOM for input range deposit amount box
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """ 
            files = glob.glob("*22WIN_%s_%s*.png"%(deposit_method,deposit_channel))
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*22WIN*.png")
        ...
        try:
            if os.path.exists(file):
                ...
                if "2WT" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="2WT")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='2WT', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets 2WT not found in file :%s"%file)
