...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "SIAM855_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('SIAM855Bot')
    ...
    logger.info("=" * 60)
    logger.info("SIAM855 PAYMENT GATEWAY TEST STARTING...")
...
async def reenter_deposit_page(page,old_url,btn,channel_btn,input_deposit_amount_box,submit_button,deposit_method,deposit_channel,min_amount,recheck,next_deposit_method):
...
async def perform_login(page):
    WEBSITE_URL = "https://www.siam855thb5.com/en-th"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.siam855thb5.com/en-th", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    try:
        ...
        await page.get_by_role("textbox", name="Enter Your Username").fill("bottesting")
    ...
    try:
        await page.get_by_role("button", name="SKIP FOR LATER").click()
    except:
...
async def url_jump_check(page,old_url,btn,channel_btn,input_deposit_amount_box,submit_button,deposit_method,deposit_channel,min_amount,telegram_message):
    try:
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await page.screenshot(path="SIAM855_%s_%s_Payment_Page.png"%(deposit_method,deposit_channel),timeout=30000)
                break 
            except TimeoutError:
                ...
                if qr_code_count != 0:
                    ...
                    if retry_count == max_retries:
                        ...
                        try:
                            await reenter_deposit_page(page,old_url,btn,channel_btn,input_deposit_amount_box,submit_button,deposit_method,deposit_channel,min_amount,recheck=1)
                        except:
    ...
    try:
        for _ in range(20):
            ...
            if await toast.count() > 0:
                toast_exist = True
                await page.screenshot(path="SIAM855_%s_%s_Payment_Page.png"%(deposit_method_text,deposit_channel),timeout=30000)
...
async def perform_payment_gateway_test(page):
    ...
    # deposit method menu 
    # class DOM: <button type="button" aria-label="Promptpay" class="deposit-method">
    
    try:
        await asyncio.sleep(5)
        old_url = page.url
        deposit_method_button = page.locator('button.deposit-method')
        ...
        if deposit_method_total_count == 0:
            raise Exception ("PERFORM PAYMENT GATEWAY TEST - DEPOSIT METHOD COUNT = 0, MENU DIDN'T LOCATE PROBABLY")
        for i in range(deposit_method_total_count):
            # because the deposit method menu anf deposit channel menu are at different page, so after checking all deposit channels of
            # that deposit method, want go back to the old url and open back the deposit menu again
            if initial_loop == False:
                await reenter_deposit_page(page,old_url,btn,channel_btn,input_deposit_amount_box,submit_button,deposit_method,deposit_channel,min_amount,recheck=0,next_deposit_method=1)
            # class DOM for deposit method text
            # <button type="button" aria-label="Promptpay" class="deposit-method">
            ...
            try:
                ...
                # deposit channel click
                # deposit channel info
                # input minimum deposit amount

                # class DOM for deposit channel button & text
                # <button type="button" aria-label="BAOYUANPAY" class="w-full flex items-center justify-between bg-content-light-2 px-4 py-3 transition-all duration-300">
                ...
                try:
                    deposit_channel_button = page.locator('button.w-full.flex.items-center.justify-between')
                    deposit_channel_total_count = await deposit_channel_button.count()
                    log.info("PERFORM PAYMENT GATEWAY TEST - DEPOSIT CHANNEL COUNT [%s]"%deposit_channel_total_count)
                    if deposit_method_total_count == 0:
                        raise Exception ("PERFORM PAYMENT GATEWAY TEST - DEPOSIT CHANNEL COUNT = 0, MENU DIDN'T LOCATE PROBABLY")
                    for j in range(deposit_channel_total_count):
                        ...
                        # class DOM: 
                        # <div class="deposit-provider-entry px-4 py-6 rounded-b-xl">
                        #      <input type="number" step="any" inputmode="numeric" class="o-input o-number mobile-big !rounded-lg md:!rounded-xl" placeholder="THB 200.00 - THB 20,000.00"><div class="mt-1 _df_ErrMsg !text-xs md:!text-sm">This field is required.</div>
                        #      <div class="mt-5 md:mt-8">
                        #           <button type="button" aria-label="Deposit" class="mx-auto !block btn btn-primary shining shrink-0 h-full uppercase" style="min-width: 280px;">Deposit</button></div></div>
                        try:
                            ...
                            url_jump, payment_page_failed_load= await url_jump_check(page,old_url,btn,channel_btn,input_deposit_amount_box,submit_button,deposit_method,deposit_channel,min_amount,telegram_message)
                            if url_jump and payment_page_failed_load == False:
                                ...
                                await reenter_deposit_page(page,old_url,btn,channel_btn,input_deposit_amount_box,submit_button,deposit_method,deposit_channel,min_amount,recheck=0,next_deposit_method=0)
                                initial_loop = False
                                continue
...
├─ **TimeOccurred:** `{timestamp}` """ 
        ...
        fail_msg = (
                "⚠️ *SIAM855 RETRY 3 TIMES FAILED*\n"
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*SIAM855*.png")
        ...
        try:
            if os.path.exists(file):
                ...
                if "S8T" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="S8T")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='S8T', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets S8T not found in file :%s"%file)
