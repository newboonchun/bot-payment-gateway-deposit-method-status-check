...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "NEX789_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('NEX789Bot')
    ...
    logger.info("=" * 60)
    logger.info("NEX789 PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.nex789v1.com/th-th"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.nex789v1.com/th-th", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    try:
        #page.get_by_role("button", name="เติมเงิน").click()
        wallet_container = page.locator('div.wallet-container-desktop')
        deposit_buttons = wallet_container.locator("button.topbar_btn_2.deposit_display_big_2", has_text="เติมเงิน")
        count = await deposit_buttons.count()
        for i in range(count):
                try:
                    deposit_button = deposit_buttons.nth(i)
                    await deposit_button.wait_for(state="visible", timeout=10000)
                    await deposit_button.click()
                    log.info("LOGIN PROCESS - DEPOSIT BUTTON (%s) ARE CLICKED"%(i+1))
                except:
                    log.info("LOGIN PROCESS - DEPOSIT BUTTON (%s) ARE FAILED TO CLICK"%(i+1))
    except Exception as e:
        log.info("LOGIN PROCESS - DEPOSIT WALLET CONTAINER FAILED TO LOCATE %s"%e)
        raise Exception("LOGIN PROCESS - DEPOSIT WALLET CONTAINER ARE FAILED CLICKED")
    #page.get_by_role("button", name="100").click()
    #page.get_by_role("button", name="3,000").click()
    #page.get_by_role("button", name="เติมเงิน").nth(2).click()
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await page.screenshot(path="NEX789_%s_%s_Payment_Page.png"%(deposit_method,deposit_channel),timeout=30000)
                break 
            except TimeoutError:
...
async def perform_payment_gateway_test(page):
    ...
    for i in range(deposit_method_total_count):
        ...
        for j in range(deposit_channel_count):
            ...
            try:
                deposit_button = page.locator('.btn_deposits')
                await deposit_button.wait_for(state="visible", timeout=60000)
                await deposit_button.click()
...
async def telegram_send_operation(telegram_message, failed_reason, program_complete):
    ...
    law_chat_id = os.getenv("LAW_CHAT_ID")
    bot = Bot(token=TOKEN)
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """ 
            files = glob.glob("*NEX789_%s_%s*.png"%(deposit_method,deposit_channel))
...
{summary_body}"""
    ...
    for attempt in range(3):
        try:
            await bot.send_message(chat_id=law_chat_id, text=caption, parse_mode='MarkdownV2', disable_web_page_preview=True)
            log.info("SUMMARY SENT")
            break
        except TimedOut:
            log.warning(f"TELEGRAM TIMEOUT，RETRY {attempt + 1}/3...")
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*NEX789*.png")
        ...
        try:
            if os.path.exists(file):
                ...
                if "N789T" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="N789T")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='N789T', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets N789T not found in file :%s"%file)
