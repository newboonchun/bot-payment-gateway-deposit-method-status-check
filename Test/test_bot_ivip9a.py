...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "IVIP9A_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('IVIP9ABot')
    ...
    logger.info("=" * 60)
    logger.info("IVIP9A PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.ivip9a.com/en-th/home"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.ivip9a.com/en-th/home", timeout=30000, wait_until="domcontentloaded")
...
async def url_jump_check(page,new_page,context,old_url,deposit_submit_button,deposit_method,deposit_channel):
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await new_page.screenshot(path="IVIP9A_%s_%s_Payment_Page.png"%(deposit_method,deposit_channel),timeout=30000)
                break 
            except TimeoutError:
    ...
    if new_payment_page == False:   
        await page.screenshot(path="IVIP9A_%s_%s_Payment_Page.png"%(deposit_method,deposit_channel),timeout=30000)
        url_jump = False
...
async def perform_payment_gateway_test(page,context):
    ...
    try:
        ...
        for i in range(deposit_options_total_count):
            ...
            # <div class="tab-header-wrapper">....<div>
            #       <span class="active">QR Pay</span>

            btn = deposit_options_button.nth(i)
            deposit_option = await btn.locator('span').inner_text()
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*IVIP9A*.png")
    ...
    if excel_len != 0:
        ...
        if os.path.exists(file):
            ...
            if "9T" in sheets:
                for attempt in range(3):
                    try:
                        df = pd.read_excel(file,sheet_name="9T")
                    except Exception as e:
                ...
                for attempt in range(3):
                    try:
                        with pd.ExcelWriter(
                            ...
                            df.to_excel(writer, sheet_name='9T', index=False)
                    except Exception as e:
                        log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                        await asyncio.sleep(5)
            else:
                log.info("Sheets 9T not found in file :%s"%file)
