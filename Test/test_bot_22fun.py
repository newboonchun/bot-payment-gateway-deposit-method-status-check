...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "22FUN_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('22FUNBot')
    ...
    logger.info("=" * 60)
    logger.info("22FUN PAYMENT GATEWAY TEST STARTING...")
...
async def reenter_deposit_page(page,old_url,btn,input_deposit_amount_box,submit_button,deposit_method,min_amount,recheck=1):
    ...
    try:
        continue_button = page.locator('button.btn.btn-bonus-continue')
        await continue_button.wait_for(state="visible")
        await continue_button.click()
        log.info("REENTER DEPOSIT PAGE - CONTINUE BUTTON ARE CLICKED")
    except:
        raise Exception("REENTER DEPOSIT PAGE - CONTINUE BUTTON ARE FAILED TO CLICK")
    if recheck:
        try:
            await btn.click()
...
async def perform_login(page):
    WEBSITE_URL = "https://www.22funthb1.com/en-th/"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.22funthb1.com/en-th/", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    try:
        ...
        log.info("LOGIN PROCESS - CONTINUE BUTTON ARE CLICKED")
    except:
        raise Exception("LOGIN PROCESS - CONTINUE BUTTON ARE FAILED TO CLICK")
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*22FUN*.png")
        ...
        try:
            if os.path.exists(file):
                ...
                if "2FT" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="2FT")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='2FT', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets 2FT not found in file :%s"%file)
