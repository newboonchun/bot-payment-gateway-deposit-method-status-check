...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "A8V2_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('A8V2Bot')
    ...
    logger.info("=" * 60)
    logger.info("A8V2 PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.aw8me.com/en-vn"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.aw8me.com/en-vn", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    await asyncio.sleep(1)
    try:
        await page.get_by_role("textbox", name="Username").fill("bottesting1")
...
async def url_jump_check(page,old_url,deposit_method,deposit_channel,bank_name,money_button_text,telegram_message):
    try:
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await page.screenshot(path="A8V2_%s_%s-%s_Payment_Page.png"%(deposit_method,deposit_channel,bank_name),timeout=30000)
                break 
            except TimeoutError:
...
async def perform_payment_gateway_test(page):
    method_exclude_list = ["Phone Card"]
    ...
    for i in range(deposit_method_total_count):
        ...
        for j in range(deposit_channel_count):
            ...
            #if deposit_channel != 'VNPAY': #FOR DEBUG
            #    continue
            ...
            try:
                ...
                for k in range(bank_counter):
                    ...
                    #url_jump, payment_page_failed_load = await url_jump_check(page,old_url,deposit_method,deposit_channel,bank_name,min_amount,telegram_message)
                    try:
                        deposit_submit_button = page.locator('button.btn_deposits.uppercase:has-text("Deposit")')
                        await deposit_submit_button.wait_for(state="visible", timeout=60000)
                        await deposit_submit_button.click()
...
async def telegram_send_operation(telegram_message, failed_reason, program_complete):
    ...
    law_vietnam_chat_id = os.getenv("LAW_VIETNAM_CHAT_ID")
    bot = Bot(token=TOKEN)
...
URL: [aw8me\\.com](https://www\\.aw8me\\.com/en\\-vn)
TEAM : A8V2
...
├─ **TimeOccurred:** `{timestamp}` """ 
            ...
            law_caption = f"""[Q121001](tg://user?id=7593954788), [aw8Ron](tg://user?id=7613770697), [Luckystar1232](tg://user?id=1255354306)
*Subject: Bot Testing Deposit Gateway*  
URL: [aw8me\\.com](https://www\\.aw8me\\.com/en\\-vn)
TEAM : A8V2
┌─ **Deposit Testing Result** ──────────┐
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """
            files = glob.glob("*A8V2_%s_%s*.png"%(deposit_method,deposit_channel))
...
{summary_body}"""
    ...
    for attempt in range(3):
        try:
            await bot.send_message(chat_id=law_vietnam_chat_id, text=caption, parse_mode='MarkdownV2', disable_web_page_preview=True)
            log.info("SUMMARY SENT")
            break
        except TimedOut:
            log.warning(f"TELEGRAM TIMEOUT，RETRY {attempt + 1}/3...")
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*A8V2*.png")
        ...
        try:
            if os.path.exists(file):
                ...
                if "A8V2" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="A8V2")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='A8V2', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets A8V2 not found in file :%s"%file)
